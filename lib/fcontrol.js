...
class fcontrol extends EventEmitter {
	...
	handleADL(data) {
		...
		this.term.log(data.ops.length + ' chat ops reported by the server.');
	}
	...
	handleFLN(data) {
		if(this.activeUsers[data.character] != undefined) {
			...
			this.term.log('User ' + data.character + 'has left.');;
		}
	}
	...
	handleLIS(data) {
		...
		try{
			...
			this.term.log(JSON.stringify(JSON.stringify(data.characters)));
		}
	}
	...
	handleNLN(data) {
		...
		this.term.log('User ' + data.identity + ' joined.')
	}
	...
	handleSTA(data) {
		...
		this.term.log('User ' + data.character + ' has updated their status.');
	}
