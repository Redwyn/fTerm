...
class simpleUI extends EventHandler{
	...
	createPane(id, name) {
		this.panes[id] = {title : name, buffer: [], curX : 0, curY : 0, mode : 0};
		this.activePane = id;
	...
	async render() {
		...
		for(let p = 0; p < paneIDs.length; p++) {
			...
			if(this.activePane != paneIDs[p]) {
				nextPane = this.panes[paneIDs[p]].title + ' ';
			} else {
				nextPane = `[${this.panes[paneIDs[p]].title}]`;
			}

			if((header + nextPane).length > this.cols) break;
			header += nextPane + (((header + nextPane).length < this.cols) ? ' ' : '');
		}
		header = colors.bgBlue(header.padEnd(this.cols));
